defmodule Atelier.AtelierLive do
  use Atelier.Web, :live_view

  import Atelier.Components.Layouts

  alias Atelier.AtelierLive.Schema
  ...
  def mount(_params, _session, socket) do
    ...
    mount_path = socket.router.__atelier_prefix__()
    ...
    {:ok,
     ...
     |> assign(
       ...
       history: [],
       history_index: 0,
       components: Atelier.ComponentsManager.list(),
  ...
  def handle_params(%{"path" => segments}, uri, socket) do
    ...
    case Atelier.ComponentsManager.read(name) do
      {:ok, data} ->
        form =
          %Schema{}
          |> Schema.changeset(%{
        ...
        {:noreply,
         assign(socket,
           ...
           component_attrs: Atelier.ComponentsManager.introspect(name),
           preview_assigns: build_default_preview_assigns(Atelier.ComponentsManager.introspect(name)),
           history: [],
           history_index: 0
         )}
  ...
  def handle_event("write", _params, socket) do
    ...
    socket =
      case Atelier.ComponentsManager.write(%{
             name: schema.name,
             html: schema.html,
  ...
  defp resolve_render_fn(name) do
    ...
    module = Module.concat(Atelier.components_module_prefix(), Macro.camelize(stem))
