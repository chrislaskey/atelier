defmodule Atelier.ComponentsManager do
  ...
  defp dir, do: Atelier.components_dir()
  defp module_prefix, do: Atelier.components_module_prefix()

  def list do
    components_dir = dir()
    File.mkdir_p!(components_dir)
    node = list_tree(components_dir, "")
  ...
  defp list_tree(current_dir, prefix) do
    case File.ls(current_dir) do
      {:ok, entries} ->
        entries = Enum.sort(entries)

        {dirs, files} =
          Enum.split_with(entries, &File.dir?(Path.join(current_dir, &1)))
        ...
        children =
          dirs
          |> Enum.map(fn entry ->
            list_tree(Path.join(current_dir, entry), prefix <> entry <> "/")
          end)
  ...
  def introspect(name) do
    ...
    module = Module.concat(module_prefix(), Macro.camelize(stem))
  ...
  def write(%{name: name} = attrs) when name != "" do
    ...
    stem = String.trim_trailing(name, ".ex")
    mod_name = "#{inspect(module_prefix())}.#{Macro.camelize(stem)}"
    path = Path.join(dir(), name)
